...
TOPICAL_ARTICLE_MAP_2026 = {
    ...
    "пытка": [20],
    "невиновность": [19],
    "презумпция невиновности": [19],
    "совесть": [25],
    "вероисповедание": [25],
    "равенство": [16],
    "дискриминация": [16],
    "налог": [38],
    "воинская обязанность": [39],
    "адвокат": [86],
    "адвокатура": [86],
    "пенсионер": [31],
    "инвалид": [31],
    "инвалидность": [31],
    "конституционный суд": [72, 73],
    "омбудсмен": [85],
    "уполномоченный по правам": [85],
    "маслихат": [88],
    "аким": [89],
    "самоуправление": [91],
    "петиция": [35],
...
def normalize_query(query: str) -> str:
    ...
    replacements = {
        ...
        # Synonym: парламент → курултай (official name in 2026 Constitution)
        "парламент": "курултай",
        "парламента": "курултай",
        "парламенту": "курултай",
        "парламентом": "курултай",
        "парламенте": "курултай",
...
def canonical_topics(query: str) -> set[str]:
    ...
    if any(x in q for x in ["семья", "дети", "ребенок", "родитель", "материнство", "отцовство", "детство", "брак"]):
        topics.add("семья")

    if any(x in q for x in ["жилище"]):
        topics.add("жилище")
...
def retrieve_keyword_priority(query: str, doc_key: str, limit: int = 5):
    ...
    # Family / children / parents keywords
    if any(x in q for x in ["семья", "дети", "ребенок", "родитель", "материнство", "отцовство", "детство", "брак"]):
        keyword_groups.append(["семья", "материнств", "отцовств", "детств", "брак"])

    # Housing keywords
    if any(x in q for x in ["жилище", "выселение"]):
        keyword_groups.append(["жилище", "неприкосновенн", "выселени"])
